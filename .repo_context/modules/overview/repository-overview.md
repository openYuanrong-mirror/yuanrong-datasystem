...
| `transfer_engine` | separate but related transfer subsystem | linked from root build when hetero and NPU-related flags are enabled; production data movement uses the CMake `-X on` auto-enabled D2D-over-HIXL backend, with legacy mode from HIXL 8.5.2+ and capability-gated CS mode from 9.1.0+ |
