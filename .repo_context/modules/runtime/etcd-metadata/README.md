...
3. Worker keeps its existing exact topology/local-notify doorbell path. If its startup topology read reports
   ...
   facts still start after the positive baseline revision `R`. The external Controller does not reconcile before the
   stream is registered; Engine then submits RESET so it exact-resyncs the read-watch window before making decisions.
...
10. `TopologyKeyHelper` classifies physical keys from the unified stream, and `TopologyEngine` applies role-delivery
    ...
    Controller validates and applies topology/membership PUT values on its serialized state thread. A membership DELETE
    exact-resyncs the complete prefix because it has no replacement value and independently dispatched same-revision
    deletes must not expose a partial membership snapshot. Worker keeps topology/local notify as exact-read doorbells,
    and task/Coordinator events retain their existing semantics.
