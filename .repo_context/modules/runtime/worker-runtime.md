...
- Shutdown:
  ...
  - topology-task ScaleIn senders accept only `ACTIVE` destinations, while receivers tolerate `PRE_LEAVING` for
    ...
    expired single-participant restart back, publication of the later stable `ACTIVE` snapshot reopens both local write
    and incoming-migration admission, including after a timed-out drain with an older request still in flight. Concurrent
    leavers cannot exchange objects after either member takes its drain snapshot.
