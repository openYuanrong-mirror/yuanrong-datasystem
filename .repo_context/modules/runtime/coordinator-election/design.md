...
## Leadership Snapshot And RPC Admission

`CoordinatorRaftNode::GetLeadershipSnapshot()` is the single observation source for role, normalized Leader address, and term. It issues one braft `Node::get_leadership_status()` call and constructs one `CoordinatorLeadershipSnapshot`; the patched O(1) accessor takes the braft Node mutex once and copies only state, term, and Leader identity without reading configuration, replicators, logs, or ballots. `CoordinatorElectionManager` and `CoordinatorServiceImpl` forward that value, while `CoordinatorRuntime::IsLeader()` and `GetLeader()` project their legacy return shapes from it. Manager, Service, and Runtime do not maintain a second mutable leadership snapshot.

For each RPC, the Service builds one complete `ResponseHeader` from that coherent snapshot. Valid routeable responses use only these states:

| Leadership/recovery observation | Header | Recovery read | Handler behavior |
| --- | --- | --- | --- |
...
`CoordinatorLeaderRouter` validates every header before updating term/Leader cache. A `NOT_LEADER` response may add its valid redirect to the current attempt set. For an ordinary RPC, `RECOVERING` is a terminal fast `K_NOT_READY` for that logical call; for a recovery-control RPC, `RECOVERING` is a route hit. `SERVING` is a route hit for both. Route hits return the RPC's business `Status` unchanged, including application errors, rather than trying another candidate. Without a Header, deterministic request statuses `K_INVALID`, `K_OUT_OF_RANGE`, `K_NOT_SUPPORTED`, and `K_DATA_INCONSISTENCY` terminate unchanged; readiness, shutdown, transport, and unknown runtime failures retain the existing candidate/Discovery/wait retry behavior.

## Physical Key Boundary

Coordinator KV RPC ingress is cluster-coordination-specific even though the Store is generic. `TopologyRecoveryManager::ParseKey()` accepts only physical keys under the default or named cluster layout that map to topology, migrate task, delete task, notify, probe, membership, ScaleIn metadata-done, or the exact eviction-policy rollout singleton. An empty relative key denotes the exact topology singleton or, for the six collection kinds, a collection prefix that remains available to canonical prefix `Range`/`WatchRange`. Service operation validation rejects `Put` and exact `DeleteRange` on those collection roots before Header, Store, or Host access. Prefix `DeleteRange` remains disallowed; the change does not broaden delete semantics. Relative-key grammar and range boundaries are validated before header construction/admission and before Store access. There is no `OTHER` key kind and no generic RPC bypass for arbitrary keys. `CoordinatorStore` itself retains general Put/Range/Delete/Watch/KeepAlive semantics; cluster scoping remains in Service/recovery admission.

## Protocol Compatibility

`ResponseHeader` now carries unified `state`, `leader_address`, `coordinator_id`, and `leader_term`; valid consumers reject unspecified/unknown states and malformed state/address combinations. This schema does not support mixed old/new producers and consumers. Rollout and rollback must move Coordinator producers and Router/Proxy consumers as one compatible set. The custom ZMQ dispatch contract is unchanged: existing methods retain their indexes, including `ReportTopologyRecoveryCandidate` 7, `GetClusterRawSnapshot` 8, and `ExchangeBootstrapObservation` 9; later methods remain append-only.
...
| Bootstrap peer channels | Bootstrap worker | A cache mutex protects per-peer reuse during parallel exchanges. After exchanges drain, the worker exit hook swaps out the cache and destroys channels outside the mutex on success, failure, or cancellation. Cached channels do not survive for the Manager lifetime. |
