...
Status TopologyPlanBuilder::BuildInterruptedScaleInRecovery(const TopologyState &latest,
                                                            const MemberIdentity &replacement,
                                                            TopologyState &next) const
{
    RETURN_IF_NOT_OK(ValidateBatch(latest, TopologyChangeType::SCALE_IN));
    ...
    for (auto &member : built.members) {
        if (member.state == MemberState::LEAVING) {
            CHECK_FAIL_RETURN_STATUS(member.identity.address == replacement.address, K_INVALID,
                                     "interrupted ScaleIn replacement address does not match leaving member");
