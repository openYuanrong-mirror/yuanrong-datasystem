...
{
    if (s == "set_local")
        return TestMode::SET_LOCAL;
    if (s == "set_remote")
        return TestMode::SET_REMOTE;
    if (s == "get_local")
        return TestMode::GET_LOCAL;
    if (s == "get_cross_node")
        return TestMode::GET_CROSS_NODE;
    if (s == "get_remote_direct")
        return TestMode::GET_REMOTE_DIRECT;
    if (s == "get_remote_cross")
        return TestMode::GET_REMOTE_CROSS;
    if (s == "mixed_local_set_get")
        return TestMode::MIXED_LOCAL_SET_GET;
    if (s == "mixed_remote_set_get")
        return TestMode::MIXED_REMOTE_SET_GET;
    if (s == "mixed_local_set_cross_get")
        return TestMode::MIXED_LOCAL_SET_CROSS_GET;
    if (s == "mixed_remote_set_remote_cross_get")
        return TestMode::MIXED_REMOTE_SET_REMOTE_CROSS_GET;
    if (s == "mset_local")
        return TestMode::MSET_LOCAL;
    if (s == "mset_remote")
        return TestMode::MSET_REMOTE;
    if (s == "mget_local")
        return TestMode::MGET_LOCAL;
    if (s == "mget_cross_node")
        return TestMode::MGET_CROSS_NODE;
    if (s == "mget_remote_direct")
        return TestMode::MGET_REMOTE_DIRECT;
    if (s == "mget_remote_cross")
        return TestMode::MGET_REMOTE_CROSS;
...
{
    return mode == TestMode::GET_LOCAL || mode == TestMode::GET_CROSS_NODE || mode == TestMode::GET_REMOTE_DIRECT
           || mode == TestMode::GET_REMOTE_CROSS || mode == TestMode::MGET_LOCAL || mode == TestMode::MGET_CROSS_NODE
           || mode == TestMode::MGET_REMOTE_DIRECT || mode == TestMode::MGET_REMOTE_CROSS;
}

bool IsMixedMode(TestMode mode)
{
    return mode == TestMode::MIXED_LOCAL_SET_GET || mode == TestMode::MIXED_REMOTE_SET_GET
           || mode == TestMode::MIXED_LOCAL_SET_CROSS_GET || mode == TestMode::MIXED_REMOTE_SET_REMOTE_CROSS_GET;
}
...
{
    return mode == TestMode::MGET_LOCAL || mode == TestMode::MGET_CROSS_NODE || mode == TestMode::MGET_REMOTE_DIRECT
           || mode == TestMode::MGET_REMOTE_CROSS;
}
...
{
    if (s == "same_keys")
        return MixedKeyStrategy::SAME_KEYS;
    if (s == "read_prev")
        return MixedKeyStrategy::READ_PREV;
    if (s == "independent")
        return MixedKeyStrategy::INDEPENDENT;
    return std::nullopt;
}
...
{
    if (s == "PREFERRED_SAME_NODE")
        return datasystem::DataPlacementPolicy::PREFERRED_SAME_NODE;
    if (s == "REQUIRED_SAME_NODE")
        return datasystem::DataPlacementPolicy::REQUIRED_SAME_NODE;
    if (s == "PREFERRED_META_OWNER")
        return datasystem::DataPlacementPolicy::PREFERRED_META_OWNER;
    return std::nullopt;
}
...
uint64_t ParseSize(const std::string &str)
{
    ...
    if (unit == "GB" || unit == "gb")
        ...
        multiplier = 1024ULL;
    else if (unit == "B" || unit == "b" || unit.empty())
        multiplier = 1;
...
bool LoadConfig(const std::string &path, Config &cfg, const std::string &outputDirOverride)
{
    ...
    try {
        ...
        if (j.contains("instance_id"))
            cfg.instanceId = j["instance_id"];
        if (j.contains("listen_port"))
            cfg.listenPort = j["listen_port"];
        if (j.contains("etcd_address"))
            cfg.etcdAddress = j["etcd_address"];
        if (j.contains("coordinator_address"))
            cfg.coordinatorAddress = j["coordinator_address"];
        if (j.contains("jf_address"))
            cfg.jfAddress = j["jf_address"];
        if (j.contains("jf_service"))
            cfg.jfService = j["jf_service"].get<std::string>();
        if (j.contains("cluster_name"))
            cfg.clusterName = j["cluster_name"];
        if (j.contains("host_id_env_name"))
            cfg.hostIdEnvName = j["host_id_env_name"];
        if (j.contains("connect_timeout_ms"))
            cfg.connectTimeoutMs = j["connect_timeout_ms"];
        ...
        if (j.contains("enable_cross_node_connection"))
            cfg.enableCrossNodeConnection = j["enable_cross_node_connection"];
        if (j.contains("batch_keys_count"))
            cfg.batchKeysCount = j["batch_keys_count"];
        if (j.contains("mset_batch_size"))
        ...
        if (j.contains("numa_node"))
            cfg.numaNode = j["numa_node"].get<int>();
        if (j.contains("random_numa_node"))
            cfg.randomNumaNode = j["random_numa_node"].get<bool>();
        if (j.contains("key_pool_size"))
            cfg.keyPoolSize = j["key_pool_size"];
        if (j.contains("inference_delay_ms"))
            cfg.inferenceDelayMs = j["inference_delay_ms"];
        if (j.contains("warmup_retry_count"))
            cfg.warmupRetryCount = j["warmup_retry_count"];
        if (j.contains("warmup_retry_delay_ms"))
            cfg.warmupRetryDelayMs = j["warmup_retry_delay_ms"];
        if (j.contains("warmup_timeout_seconds"))
            cfg.warmupTimeoutSeconds = j["warmup_timeout_seconds"];
        if (j.contains("max_key_pool_size"))
            cfg.maxKeyPoolSize = j["max_key_pool_size"];
        if (j.contains("target_hit_rate"))
            cfg.targetHitRate = j["target_hit_rate"];
        if (j.contains("metrics_interval_ms"))
            cfg.metricsIntervalMs = j["metrics_interval_ms"];
        if (j.contains("metrics_file"))
        ...
        if (cfg.peers.empty() && !cfg.nodes.empty()) {
            for (auto &n : cfg.nodes) {
                if (n.instanceId == cfg.instanceId)
                    continue;
        ...
        if (j.contains("role"))
            cfg.role = j["role"].get<std::string>();
        ...
        if (j.contains("cleanup_method"))
            cfg.cleanupMethod = j["cleanup_method"].get<std::string>();
        if (j.contains("remote_worker") && j["remote_worker"].is_object()) {
            const auto &rw = j["remote_worker"];
            if (rw.contains("host"))
                cfg.remoteWorker.host = rw["host"].get<std::string>();
            if (rw.contains("port"))
                cfg.remoteWorker.port = rw["port"];
        }
        if (j.contains("connect_options") && j["connect_options"].is_object()) {
            const auto &co = j["connect_options"];
            if (co.contains("connect_timeout_ms"))
                cfg.connectTimeoutMs = co["connect_timeout_ms"];
            if (co.contains("request_timeout_ms"))
                cfg.requestTimeoutMs = co["request_timeout_ms"];
            if (co.contains("enable_cross_node_connection"))
                cfg.enableCrossNodeConnection = co["enable_cross_node_connection"];
            if (co.contains("enable_local_cache"))
                cfg.enableLocalCache = co["enable_local_cache"];
        ...
        if (j.contains("mode") && j["mode"].is_string()) {
            ...
            if (!parsed.has_value()) {
                SLOG_ERROR("Unknown mode '" << j["mode"].get<std::string>() << "', must be pipeline/cache/benchmark");
                return false;
            }
    ...
    if (cfg.etcdAddress.empty() && cfg.coordinatorAddress.empty() && cfg.jfAddress.empty()) {
        SLOG_ERROR("either etcd_address or coordinator_address or jf_address is required");
        return false;
    }
    ...
    if (cfg.verifyLevel != "off" && cfg.verifyLevel != "size" && cfg.verifyLevel != "sample"
        && cfg.verifyLevel != "full") {
        SLOG_ERROR("verify.level must be one of off/size/sample/full, got '" << cfg.verifyLevel << "'");
        return false;
    }
    ...
    if (cfg.maxKeyPoolSize > 0 && cfg.maxKeyPoolSize < static_cast<uint64_t>(cfg.keyPoolSize)) {
        SLOG_ERROR("max_key_pool_size (" << cfg.maxKeyPoolSize << ") must be >= key_pool_size (" << cfg.keyPoolSize
                                         << ")");
        return false;
    }
    ...
    if (cfg.runMode == RunMode::BENCHMARK) {
        ...
        if (IsMixedMode(cfg.testMode)) {
            ...
            if (cfg.mixedKeyStrategy == MixedKeyStrategy::INDEPENDENT && cfg.cleanupMethod == "ttl") {
                SLOG_ERROR(
                    "independent strategy is incompatible with cleanup_method=ttl "
    ...
    log << "Config loaded: mode=" << runModeNames[static_cast<int>(cfg.runMode)] << ", instance_id=" << cfg.instanceId
        << ", port=" << cfg.listenPort << ", etcd=" << cfg.etcdAddress << ", coordinator=" << cfg.coordinatorAddress;
    if (cfg.testMode != TestMode::NONE) {
        ...
        log << ", role=" << cfg.role << ", pipeline=[" << joinStr(cfg.pipeline) << "]"
            << ", notify_pipeline=[" << joinStr(cfg.notifyPipeline) << "]"
            << ", nodes=" << cfg.nodes.size() << ", peers=" << cfg.peers.size() << ", target_qps=" << cfg.targetQps
