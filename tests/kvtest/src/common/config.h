...
struct Config {
    ...
    std::string jfAddress;                          // JF mock address, when set use JF discovery
    std::string jfService = "kvcache_coordinator";  // JF service name
    ...
    bool randomNumaNode =
        false;  // true = let kvtest pick a NUMA node at random at startup; mutually exclusive with numaNode >= 0
    // Cache mode fields
