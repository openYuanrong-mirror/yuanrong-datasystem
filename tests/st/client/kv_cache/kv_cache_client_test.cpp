...
TEST_F(KVCacheClientTest, TestFirstStubConnectFailed)
{
    ...
    // Only the direct-port-disable inject remains for stub-connect failure.
...
TEST_F(KVCacheClientTest, CreateSetAndGetUseSharedMemoryWhilePinPending)
{
    ...
    SetParam param{ .writeMode = WriteMode::NONE_L2_CACHE };
    const std::string singleKey = "pin_pending_single_" + client->GenerateKey();
    ...
    std::vector<std::string> keys{ "pin_pending_batch_0_" + client->GenerateKey(),
                                   "pin_pending_batch_1_" + client->GenerateKey() };
