...
TEST(TopologyPlanBuilderTest, InterruptedScaleInRestoresCommittedOwners)
{
    ...
    const MemberIdentity replacement{ std::string(16, 'd'), latest.members[0].identity.address };
    DS_ASSERT_OK(builder.BuildInterruptedScaleInRecovery(latest, replacement, recovered));

    EXPECT_EQ(recovered.version, 5);
    EXPECT_FALSE(recovered.activeBatch.has_value());
    ASSERT_EQ(recovered.members.size(), 3);
    EXPECT_EQ(recovered.members[0].identity, replacement);
...
TEST(TopologyPlanBuilderTest, InterruptedScaleInRejectsMultipleLiveExecutors)
{
    ...
    TopologyState recovered;
    EXPECT_TRUE(builder.BuildInterruptedScaleInRecovery(latest, latest.members[0].identity, recovered).IsError());
}
