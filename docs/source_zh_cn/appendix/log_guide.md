...
9. **指标摘要日志（kv_metrics）**：Worker、Coordinator 和 SDK Client 共用的 `metrics_summary` 摘要 JSON-Lines 输出，每行一个完整 JSON 对象，顶层含 `time`/`pod_name`/`cluster_name`，body 与原 `metrics_summary` 摘要格式一致（需 `json_log_monitor`；开启且 JSON exporter 创建成功后，`kv_metrics.log` 每周期完整记录，主 INFO 不再重复输出 `metrics_summary`；JSON exporter 未启用或未创建时，`log_monitor` 控制的 INFO 输出保持原行为）。Worker 输出资源和数据面指标，Coordinator 输出 RPC、Watch 分发及通知指标，SDK Client 输出客户端请求、错误和时延指标。
...
| 9 | datasystem_worker | `/path/yr_datasystem/logs/kv_metrics.log` | 指标摘要 JSON-Lines；body 与原 `metrics_summary` 摘要格式一致，额外前置 `time`/`pod_name`/`cluster_name`；由 `json_log_monitor` 控制。开启后 `kv_metrics.log` 每周期完整记录，主 INFO 不再重复输出 `metrics_summary`；JSON exporter 未启用或未创建时保持 `log_monitor` 控制的 INFO 输出 |
...
| 13 | Client | `/path/client/kv_metrics.log` | SDK Client 请求、错误和时延指标的 JSON-Lines 摘要；`ObjectClient` 与 `KVClient` 初始化同一 metrics catalog。`json_log_monitor` 开启且 exporter 创建成功后，每周期完整写入该文件且不再向 Client INFO 重复输出；exporter 未启用或未创建时保持 INFO fallback |
| 14 | datasystem_coordinator | `/path/yr_datasystem/logs/datasystem_coordinator.INFO.log`（及 `.WARNING`、`.ERROR` 等轮转文件） | Coordinator 运行日志；启动时记录 Git Commit 和 Branch |
| 15 | datasystem_coordinator | `/path/yr_datasystem/logs/datasystem_coordinator_operation.log` | Coordinator 操作审计日志；记录 Init/Shutdown、构建 Commit、配置初始化及运行时 `UpdateConfig` 变更 |
| 16 | datasystem_coordinator | `/path/yr_datasystem/logs/kv_metrics.log` | Coordinator RPC、Watch 分发及通知指标的 JSON-Lines 摘要；由 `json_log_monitor` 控制。开启且 exporter 创建成功后 `kv_metrics.log` 每周期完整记录，主 INFO 不再重复输出 `metrics_summary`；JSON exporter 未启用或未创建时保持 `log_monitor` 控制的 INFO 输出 |
